<template>
    ...
    <section v-else>
        <div
            v-if="total == 0"
            class="defi-empty">
            <i class="fa fa-user-secret defi-empty__icon"/>
            <p class="defi-empty__description">No account found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">{{ _nFormatNumber('account', 'accounts', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--accounts">
            ...
            <template
                slot="balance"
                slot-scope="props">
                <span class="d-lg-none">{{ formatUnit(toDefi(props.item.balance, 5)) }}</span>
                <span class="d-none d-lg-block">{{ formatUnit(toDefi(props.item.balance)) }}</span>
            </template>
        </table-base>
