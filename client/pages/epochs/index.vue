<template>
    ...
    <section v-else>
        <div
            v-if="total == 0"
            class="defi-empty">
            <i class="fa fa-cubes defi-empty__icon"/>
            <p class="defi-empty__description">No Epoch found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">{{ _nFormatNumber('epoch', 'epochs', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--epochs">
