<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="defi-empty">
            <i class="fa fa-exchange defi-empty__icon"/>
            <p class="defi-empty__description">No transaction found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">{{ _nFormatNumber('transaction', 'transactions', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--transactions">
            <template
                slot="hash"
                slot-scope="props">
                <nuxt-link
            ...
            <template
                slot="value"
                slot-scope="props">{{ formatUnit(toDefi(props.item.value)) }}</template>
            ...
            <template
                ...
                <span
                    v-b-tooltip.hover
                    :title="formatUnit(toDefi(props.item.gasPrice * props.item.gasUsed, 18))">
                    {{ formatUnit(toDefi(props.item.gasPrice * props.item.gasUsed, 8)) }}</span></template>
        </table-base>
