<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="defi-empty">
            <i class="fa fa-binoculars defi-empty__icon"/>
            <p class="defi-empty__description">No event found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">{{ _nFormatNumber('item', 'items', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--events">
