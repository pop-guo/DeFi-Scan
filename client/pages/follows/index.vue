<template>
    ...
    <section
        v-else
        class="defi-body-wrapper defi-body-wrapper--follow">
        <div class="mb-4">
            <b-btn
            ...
            <b-modal
                id="modalAddFollow"
                ref="modalNewAddress"
                class="defi-modal defi-modal--follow"
                ...
                on your follow list receives an incoming DEFI Transaction.</p>
                <div class="form-group">
                    <div class="defi-toggle">
                        <input
                            id="emailNotify"
                            ...
                            name="sendEmail"
                            type="checkbox"
                            class="defi-toggle__checkbox">
                        <label
                            for="emailNotify"
                            class="defi-toggle__btn"/>
                        <span class="defi-toggle__label"><i class="fa fa-envelope-o mr-2"/>Email Notification</span>
                    </div>
                    <div
                        v-if="formSendEmail"
                        class="defi-toggle">
        ...
        <div
            v-if="total == 0"
            class="defi-empty">
            <i class="fa fa-chain-broken defi-empty__icon"/>
            <p class="defi-empty__description">No address found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">Total {{ formatNumber(total) }} items found</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--follow">
            ...
            <template
                slot="balance"
                slot-scope="props">
                <span class="d-lg-none">{{ formatUnit(toDefi(props.item.addressObj.balance, 5)) }}</span>
                <span class="d-none d-lg-block">{{ formatUnit(toDefi(props.item.addressObj.balance)) }}</span>
            </template>
