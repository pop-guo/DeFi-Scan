<template>
    ...
    <section v-else>
        ...
        <div
            ...
            <p class="defi-empty__description">No token found</p>
        </div>

        <p
            v-if="total > 0"
            class="defi-total-items">{{ _nFormatNumber('token', 'tokens', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="defi-table--tokens-nft">
...
export default {
    ...
    mounted () {
        ...
        this.$store.commit('breadcrumb/setItems', { name: 'tokens', to: { name: 'tokens-trc21' } })
