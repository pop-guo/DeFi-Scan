<template>
    ...
    <div
        v-else
        class="card defi-card defi-card--contract-info">

        <div
            v-if="data.length == 0"
            class="defi-empty">
            <i class="fa fa-book defi-empty__icon"/>
            <p class="defi-empty__description">Can not read this smart contract</p>
        </div>

        <div class="defi-card__header mb-3">
        ...
        <table class="defi-card__table defi-contract-info">
            <tr
                v-for="(func, index) in data"
                :key="index">
                <td :class="`defi-contract-info__func ${func.inputs.length ? 'with-input' : 'no-input'}`">
                    <span class="defi-contract-info__name">
                    ...
                    <span
                        v-if="typeof func.result !== 'undefined' && ! func.inputs.length"
                        class="defi-contract-info__result">
                        <nuxt-link
                        ...
                        <em class="defi-contract-info__type">{{ func.outputs[0].type }}</em>
                    </span>
                    <div
                        v-if="func.inputs.length"
                        class="defi-contract-info__inputs">
                        <input
                    ...
                    <div
                        ...
                        class="defi-contract-info__outputs">
                        <i class="fa fa-angle-double-right"/>
                        <span
                            v-for="(output, idx2) in func.outputs"
                            :key="idx2">
                            <span class="defi-contract-info__param-name">{{ output.name }}</span>
                            <em class="defi-contract-info__type">{{ output.type }}</em>
    ...
    methods: {
        ...
        async callFunction (functionName, signature, inputElement, outputElement) {
            ...
            document.getElementById(outputElement).innerHTML =
                `<div class="defi-contract-info__response">
        ...
        formatOuputs (output) {
            ...
            for (let i = 0; i < output.length; i++) {
                ...
                response += `<strong>${output[i].name}</strong>&nbsp;
                    <em class="defi-contract-info__type">${output[i].type}</em> :
        ...
        resetInput (e) {
            e.preventDefault()
            let inputs = document.querySelectorAll('.defi-contract-info input')
            let ouputs = document.querySelectorAll('.defi-contract-info__func.with-input .defi-contract-info__result')
